...
const AddCreatorProfile = ({ addCardStatus, getAddCardData, handleLoaderStatus }) => {
    ...
    useEffect(() => {
        console.log("Add card status ::::", addCardStatus);
        if (addCardStatus === true) {
            handleAddCard();
            console.log("Add card status12222 ::::", addCardStatus);
        ...
        stripeReact.createToken(cardNumberElement).then(async function (tok) {
            if (tok.error) {
                ...
                try {
                    ...
                    console.log("Token for add card ", AuthToken);
...
export default AddCreatorProfile